timeSlider <- function(inputId = "timeRange", min, max, value) {
  sliderInput(inputId, label = "Time range (converted to rounds in display items)",
              min = min, max = max, value = c(max - months(1), max), width = "95%")
}
